...
RemoteStorage.defineModule('credentials', function(priv, pub){
  ...
  Credentials.prototype = {
    ...
    _getKey: function(doCrypt) {
      var promise = promising();
      this._keyHandlers.push(promise)
      var cb = function(key) {
        this.secret = key;
        while(this._keyHandlers.length > 0) {
          this._keyHandlers.pop().fulfill();
        }
      }.bind(this)
      this.getKeyCb(cb);
      return promise;
    ...
    personas: function() {
      ...
      function oneFailed(e) {
        console.error('getting Personas one Failed ', e, e.stack);
        oneDone();
      }
      priv.getListing('').then(function(listing) {
        ...
        listing.forEach(function(platform) {
          if (platform.slice(-1) !== '/')
            return;
          n += 1;
          return priv.getListing(platform).then(function(personas) {
            n+=1;
    ...
    add: function(platform, persona, data) { 
      if(!platform || !persona)
        return promising().reject("no persona or platform")
